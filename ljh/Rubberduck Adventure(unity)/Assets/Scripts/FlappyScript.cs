...
public class FlappyScript : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (GameStateManager.GameState == GameState.Intro)
        {
            if (GetComponent<Rigidbody2D>().velocity.y < -1) //스피드가 떨어질때 
    ...
    private void FixFlappyRotation()
    {
        ...
        //clamp 값 -90<rotation<45 
        birdRotation = new Vector3(0, 0, Mathf.Clamp(birdRotation.z + degreesToAdd, -45, 30));
        transform.eulerAngles = birdRotation; //오일러 공식 이용
    }
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        ...
        {
            if (col.gameObject.tag == "Pipeblank") //pipeblank 파이프 사이 공간 
            {
