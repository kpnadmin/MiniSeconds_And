package com.spring63.rest.controller;

import org.json.JSONArray;
import org.json.JSONException;
import org.json.JSONObject;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.RequestParam;
import org.springframework.web.bind.annotation.ResponseBody;

/**
 * Handles requests for the application home page.
 */
@Controller
public class JsonController {
	
	private static final Logger logger = LoggerFactory.getLogger(JsonController.class);
	

    public String getRandString() {
        String str = "";
        
        for (int i = 1; i <= (int) (Math.random()*10000); i++) {
            //char ch = (char) ((Math.random() * 11172) + 0xAC00);
            str += String.valueOf( (char) ((Math.random() * 26) + 97) );
          }
        
        return str;
    }
    

    @RequestMapping(value = "/json/JSONServer", method = RequestMethod.GET)
    public @ResponseBody  String JSONServer( @RequestParam(value="msg", required = false, defaultValue = "") String msg
                                          , Model model) throws Throwable 
    {
        logger.info("JsonController.JSONServer");

        JSONObject jsonMain = new JSONObject();
        JSONArray jArray = new JSONArray();
        
        JSONObject jObject = new JSONObject();

        jObject.put("msg1", msg);
        jObject.put("msg2", "메시지2!");
        jObject.put("msg3", "3번째 메시지!");
        
        jArray.put(jObject);

        jsonMain.put("List", jArray);
        
        return jsonMain.toString() ;
    }
    

	@RequestMapping(value = "/json/ContactWS", method = RequestMethod.GET)
	public @ResponseBody String ContactWS( @RequestParam(value="start", required = false, defaultValue = "0") String start
	                                      , @RequestParam(value="paging", required = false, defaultValue = "10") String paging
	                                      , Model model) throws Throwable 
    {
		logger.info("JsonController.ContactWS");
		   
	    int cntStart  = 0;
	    int cntPaging = 100;
	    int cntLast   = 0;
	    
	    if(paging != "" && paging != null) {
	        cntPaging = Integer.parseInt(paging);
	    }
	    
	    if(start != "" && start != null) {
	        cntStart = Integer.parseInt(start);        
	    }
	    
	    cntLast = cntStart + cntPaging;
	    
	    
	    String result = "";

	    try {
            JSONObject jsonMain = new JSONObject();
            JSONArray jArray = new JSONArray();
            
            JSONObject jObject = null;

            for(int i = cntStart ; i<= cntLast; i++) {
                jObject = new JSONObject();
                jObject.put("name"    , "name : "     + String.valueOf(i) ); 
                jObject.put("surname" , "surname : "  + String.valueOf(i) ); 
                jObject.put("email"   , "email : "    + String.valueOf(i) ); 
                jObject.put("phoneNum", "phoneNum : " + String.valueOf(i) );    
                
                jArray.put(jObject);    
            }

            jsonMain.put("List", jArray);   
            
            result = jsonMain.toString();
        } catch (JSONException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();            
        }
	    
	    
	    return result ;
	}

    @RequestMapping(value = "/json/ModelItem", method = RequestMethod.GET)
    public @ResponseBody  String ModelItem( @RequestParam(value="start", required = false, defaultValue = "0") String start
                                          , @RequestParam(value="paging", required = false, defaultValue = "10") String paging
                                          , Model model) throws Throwable 
    {
        logger.info("JsonController.JSONServer");

        int cntStart  = 0;
        int cntPaging = 100;
        int cntLast   = 0;
        

        if(paging != "" && paging != null) {
            cntPaging = Integer.parseInt(paging);
        }
        
        if(start != "" && start != null) {
            cntStart = Integer.parseInt(start);        
        }
        
        cntLast = cntStart + cntPaging;
        

        JSONObject jsonMain = new JSONObject();
        JSONArray jArray = new JSONArray();
        
        JSONObject jObject = null;
        
        if(cntLast < cntStart ) {
            int temp = cntLast;
            cntLast = cntStart;
            cntStart = temp;
        }

        for(int i = cntLast  ; i>= cntStart; i--) {
            jObject = new JSONObject();
            jObject.put("iconItem"  , "" ); 
            jObject.put("dataItem01", "추억의 테트리스" ); 
            
            jObject.put("dataItem02", getRandString() ); 
            jObject.put("dataItem03", String.valueOf(i) + " 원" );   
            jObject.put("dataItemID", String.valueOf(i) );   
            
            jArray.put(jObject);    
        }

        jsonMain.put("List", jArray);   
            
        return jsonMain.toString();
    }
    
    
	
}
